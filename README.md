...
The covariance is evaluated from the formula above. The Wiener integral in the mean is approximated from a discrete past path using step functions.
...
K = np.array([
    [r(H, t, s) for s in past_times[1:]]
    for t in past_times[1:]
])

past_values = np.r_[
    0.0,
    rng.multivariate_normal(np.zeros(100), K)
]

# Conditional law of future values.
future_times = np.linspace(u, 2.0, 11)[1:]

mean, cov = conditional_law(
    H,
    future_times,
    past_times,
    past_values
